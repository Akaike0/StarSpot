...
namespace StarSpot
{
    class Grinding
    {
        ...
        private bool mob_bugged_check = false;
        ...
        public void healing()
        {
            ...
            else if (stats.player_aggro() == 0 && Properties.Settings.Default.healing_under_np == 0)
            {
                ...
                {
                    // Log
                    MainWindow.log_text = "Healing is deactivated.";
        ...
        public void walking()
        {
            if (stats.player_health() != 0) // Make sure this works only if the player isn't dead
            {
                ...
                {
                    if ((TargetInfo.target_tid == 0 && stats.player_aggro() == 0) | (Blacklist.blacklisted && stats.player_aggro() == 0) | Properties.Settings.Default.enable_autoselling_ignore_mobs) // Only if player has no aggro
                    {
                        autoselling();
                    }
        ...
        {
            // Stats requirements
            if (!player_attacking && (stats.player_targetclass() != 2 | Properties.Settings.Default.aim_friendly_target) && (!player_ressurection | (player_ressurection && (Properties.Settings.Default.enable_tabbing_after_death | Properties.Settings.Default.enable_defending_after_death))) && (!player_autoselling | (player_autoselling && !Properties.Settings.Default.enable_autoselling_ignore_mobs)) ) // Attacking
            {
                if (TargetInfo.target_id != 0 | stats.player_aggro() != 0)
                {
                    if ((!Blacklist.blacklisted && (TargetInfo.target_target_tid == 0 | Properties.Settings.Default.attack_mobs_from_other_players)) | (TargetInfo.target_target_tid == stats.player_id() && TargetInfo.target_aggro != 0) | stats.player_aggro() != 0)
                    {
                        if (TargetInfo.target_id != 0)
                        {
                            // Log
            ...
            else if (stats.player_aggro() == 0 && player_attacking) // Looting
            {
            ...
            else if ((player_walking && !player_healing) | (unstucking_now && !player_healing && stats.player_aggro() == 0))
            {
                // Reset Counter
                SkillsSystem.skills_counter = 1;
        ...
        {
            if (TargetInfo.target_tid != 0 && (TargetInfo.target_aggro == 0 | TargetInfo.target_targets_id != stats.player_id()))
            {
                // Log
